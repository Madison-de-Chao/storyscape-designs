  ...
  const renderSlot = (slot: SaveSlot) => (
    <motion.div
      ...
      className={`
        ...
        hover:border-stone-500/70 hover:bg-stone-800/50
        group
      `}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            {slot.isAutoSave ? (
              <span className="text-xs px-2 py-0.5 rounded bg-amber-700/50 text-amber-200">
                自動存檔
              </span>
            ) : (
        ...
        <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {mode === 'load' && (
            <Button
              size="sm"
              variant="ghost"
              className="h-8 px-3 text-emerald-400 hover:text-emerald-300 hover:bg-emerald-900/30"
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-lg bg-gradient-to-b from-stone-900 to-stone-950 rounded-2xl border border-stone-700/50 shadow-2xl overflow-hidden"
